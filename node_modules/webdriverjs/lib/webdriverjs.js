...
var WebdriverJs = function(options)//host, port, username, pass) 
{
	...
	// this function is an entry point for adding new commands
	...
	var QueueItem = function(commandName, method, scope, args)
	{
		...
		this.isDone = false;
		this.handlingErrors = commandName == 'handleError';
				...
				this.arguments.push((function(method, methodName)
					{
						...
						{
							if (catchErrors && !self.handlingErrors) {
								if (caughtError === undefined) {
									try {
										// call the callback
										method.call(scope, result);
									} catch (e) {
										// save the error for retrieval
										caughtError = e;
									}
								}
							} else {
...
//					console.log("NEXT IN EMPTY CALLBACK")
	...
	QueueItem.prototype.run = function()
	{
		...
		if (currentLength < this.children.length && (!this.handlingErrors || caughtError))
		{
			//console.log("RUM")
			this.next();
		}
	...
	if (catchErrors) {
		this.addCommand('handleError', function(callback) {
			var err = caughtError;
